...
plt.style.use("aps")
mpl.rcParams["figure.figsize"] = [1.5*3.4039, 1.5*2.10373]
rm = 2.9673;
...
with plt.style.context('aps'):
    ...
    plt.ylabel(r'V[K]')
    plt.xlabel(r'x[$\AA$]')
    plt.plot(xval2,V_shell_Ar(xval2)+PIMC_potential_MCM(xval2,15.51),label=r'$U_{Ar/MCM}(r)$')
    plt.plot(xval,V_shell(xval)+PIMC_potential_MCM(xval,15.51),label=r'$U_{Cs/MCM}(r)$')
    #plt.plot(xval2,PIMC_potential_MCM(xval2,15.51),label=r'$U(r)}$')
    plt.plot(xval2,PIMC_potential_MCM(xval2,15.51), label=r'$U_{MCM}$')
    plt.legend()
