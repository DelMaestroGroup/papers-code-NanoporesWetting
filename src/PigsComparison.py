...
#with open('Radial-wf-Ar4.npy','wb') as f:
#    np.save(f,radial)
#with open('Radval-wf-Ar4.npy','wb') as f:
#    np.save(f,rval)

norm = normalize_psi_PIMC(radial,rval)
ax.plot(rval,radial, '*-', label="Relaxation",color='#5E4FA2',alpha=1.0)

f2 = open('radial-N-reduce-87e3247b-4b29-4e2a-998d-9e43a48f3593.dat','r')
lines = f2.readlines()
x = np.array([])
y = np.array([])
z = np.array([])
...
ax.errorbar(x,y,yerr=z, linestyle="None",marker = '+',label='PIMC',color='#5E4FA2',alpha=0.5)
ax.errorbar(x,2*y/norm,yerr=2*z/norm, linestyle="None",marker = '+', label='Re-normalised PIMC',color='#5E4FA2',alpha=0.7)
